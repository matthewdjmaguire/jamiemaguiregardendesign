...
## Content model
- Portfolio projects live in `src/content/projects/*.md` (frontmatter: title, location, summary, order, images[] with alt text; the first image is the cover). Photos live in `src/assets/photos/`. Keep this structure so a Git-based CMS can be added later.
- Site-wide facts (name, email, social links, nav, the `headerStyle` toggle: `'solid'` or `'overlay'`, and the `logo` toggles for header/footer/favicon) live in `src/data/site.ts`; logo files are in `public/brand/` and are generated by `design/logo/build_logo.py`; colours, type and spacing live in `src/styles/tokens.css`. Change those, not individual components.
