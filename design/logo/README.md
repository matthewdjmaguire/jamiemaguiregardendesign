...
Run from the repo root with a Python that has `fonttools`, `brotli`, `uharfbuzz`, `pillow` and `pymupdf` installed (they are design tools only, not dependencies of the website):

```bash
python3 -m venv ~/Developer/.jmgd-work/venv
~/Developer/.jmgd-work/venv/bin/pip install fonttools brotli uharfbuzz pillow pymupdf
